        ...
        when(tabType) {
            0 ->{
                if (session?.userToken.equals("0")){
                    root = inflater.inflate(R.layout.fragment_register_user, container, false)
                    ...
                    root= inflater.inflate(R.layout.fragment_update_safety, container, false)

                    val btnSellGroceries: Button = root.findViewById(R.id.btn_sell_on_cw)
                    btnSellGroceries.setOnClickListener {
                        context!!.startActivity(Intent(context, VendorActivity::class.java))
                    }
